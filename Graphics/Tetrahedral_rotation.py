...
def get_vector(num, point_list):
    # 默认的面顺序为ABC，ADC，ABD，BCD，输出当前面的点列表和数据列表及法向量
    # 拿个中心点一会算法向量
...
def get_panner_flag(view_point, point_list):
    ...
    for i in range(1, 5):
        ...
        print("In panner ", i, "The n is:", n, "The v is:", v)
    # n_list=[[-18,9,18],[0,-18,0],[18,9,18],[0,0,-36]]# 验证是否是法向量出错
...
def line_can_see(eye_flag, point_list):
    ...
    # 每条边的可见性由面决定，如果面不可见则所有的边都减一
    # 则在每个不可见的面的边表中遍历所有的边，并用边的正值和相反值检索真边表中是否有相同的，有的话真边表的那个可见性flag-1
    ...
    print("We can see line is real_point_list_with_eyeflag:",
          real_point_list_with_eyeflag)
    return real_point_list_with_eyeflag
...
def draw_all_line(point_list, eye_flag, ax):
    ...
    for i in range(len(see_line_list)):
        ...
        draw_line_between_two_point(
            see_line_list[i][0], see_line_list[i][1], point_list, ax)
    for j in range(len(notsee_line_list)):
        draw_line_between_two_point(
            notsee_line_list[j][0], notsee_line_list[j][1], point_list, ax, linecolor='g', linestyle='--')


def draw_tetrahedral(view_point, inner, point_list):
    ...
    # 画边
    draw_all_line(point_list, eye_flag, ax)
    plt.show()
...
def rotate(point_list,theta):
    ...
    # for i in range(4):
    #     point=point_list[i]
    #     new_point_list[i]=np.dot(point,rotate_matrix)
...
def fun():
    ...
    draw_tetrahedral(view_point, inner, point_list)
    # 旋转后的
    rotated_point_list=rotate(point_list,theta)
    rotated_inner = get_inner_point(rotated_point_list)
    draw_tetrahedral(view_point, rotated_inner, rotated_point_list)
